...
def split_into_chunks(text):
  size = len(text.split(' ')) # use the length/size of our input data to determine a suitable range for max_tokens in the next step
  ...
  chunks = splitter.chunks(text) # splits the text into chunks using the BERT model above
  return chunks # returns a list of chunks

def summarise_chunks(chunks):
  full_summary = {}
  for i, chunk in enumerate(chunks): # summarise each chunk produced by the semantic text splitter
    ...
    full_summary[i] = summary
    #print(f"Chunk {i}: {chunk}\nSummary: {summary}")
  return full_summary # returns a dictionary of chunks and their order/index
